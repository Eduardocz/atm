...
class AccountService {
    ...
    async findOneByNumber(numberAccount) {
    ...
    async deposit(numberAccount, amount) {
        let rtaAccount = await this.findOneByNumber(numberAccount);
    ...
    async withdraw(numberAccount, amount) {
        let amountFee = 0
        let rtaAccount = await this.findOneByNumber(numberAccount);
        let balance = rtaAccount.dataValues.balance;
    ...
    async payCredit(numberAccount, amount){
        let rtaAccount = await this.findOneByNumber(numberAccount);
        let balance = rtaAccount.dataValues.balance;
